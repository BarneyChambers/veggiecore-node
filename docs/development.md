...
To develop bitcore-node-veggie:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-veggie.git
...
We now will setup symlinks in `bitcore-node-veggie` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-veggie
ln -s ~/bitcoind-rpc-veggie
```
...
cd devnode
mkdir node_modules
touch bitcore-node-veggie.json
touch package.json
```

Edit `bitcore-node-veggie.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-veggie/bin/bitcore-node-veggie start
```